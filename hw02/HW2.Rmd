...
## 5. Make Scatterplot
In general, the states that poverty crude prevalence under 20, the poverty crude prevalence has a negative correlation with binge drinking overall crude prevalence. In other words, states that are more wealthy overall binge drinking more. However, there are few states that scored extremely high in poverty, like Puerto Rico; therefore, the smooth method has less confidence in predicting the overall binge drinking rate for poverty crude prevalence over 30. This could be seen in the gradually wider confidence interval (grey area) in the plot.
...
Top5Increase_AGR <- question6_df %>%
  arrange (state) %>%
  group_by(state) %>%
  summarize (AverageAnnualGrowthRate = mean (binge_all)) %>%
  top_n(n=5) %>%
  arrange(desc(AverageAnnualGrowthRate))

#calculate AGR for binge_all and select the smallest 5 AGR binge_all
Top5Decrease_AGR <- question6_df %>%
  arrange (state) %>%
  group_by(state) %>%
  summarize (AverageAnnualGrowthRate = mean (binge_all)) %>%
  top_n(n=-5) %>%
...
View (Top5Increase_AGR)
View (Top5Decrease_AGR)
```
